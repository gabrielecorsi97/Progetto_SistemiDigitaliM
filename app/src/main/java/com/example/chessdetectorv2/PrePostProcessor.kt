...
class Result(var classIndex: Int,
             var score: Float,
             var rect: Rect,
             var classes_scores_list : MutableList<Pair<Int,Float>>)
...
object PrePostProcessor {
    ...
    private const val mOutputColumn = 17 // left, top, right, bottom, score and 12 class probability
    private const val mThreshold = 0.50f // score above which a detection is generated
    ...
    fun outputsToNMSPredictions(outputs: FloatArray, imgScaleX: Float, imgScaleY: Float, ivScaleX: Float,
        ivScaleY: Float, startX: Float, startY: Float): ArrayList<Result> {
        ...
        for (i in 0 until mOutputRow) {
            if (outputs[i * mOutputColumn + 4] > mThreshold) {
                ...
                val classes_scores = mutableListOf<Pair<Int,Float>>()
                for (j in 0 until mOutputColumn - 5) {
                    classes_scores.add(Pair(j,outputs[i * mOutputColumn + 5 + j]))

                }
                classes_scores.sortWith(compareByDescending { it.second })
